...
if (w.equals("in")) {	// 일정 등록일 경우
	msg = "등록";
	y = Integer.parseInt(request.getParameter("y"));	// 연
	m = Integer.parseInt(request.getParameter("m"));	// 월
	d = Integer.parseInt(request.getParameter("d"));	// 일
	h = curTime.get(Calendar.HOUR_OF_DAY);	// 현재 시각(0~23)
	n = curTime.get(Calendar.MINUTE);		// 현재 분(0~59)
	...
	try {
		...
		String sql = "select * from t_schedule_list " + 
			"where ml_id = '" + uid + "' and sl_idx = "  + idx;
