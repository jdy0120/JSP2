...
// 로그인에 대한 처리작업을 실행하는 클래스(DB처리 제외)
	public MemberInfo getLoginMember(String uid, String pwd) {
		...
		// 처리 결과를 MemberInfo형 인스턴스로 리턴해줌
		LoginDao loginDao = LoginDao.getInstance();
		// LoginDao 형 인스턴스 loginDao를 선언 및 생성
		Connection conn = getConnection();
		// JdbcUtil 클래스의 getConnection() 메소드를 이용하여 Connection 객체 conn생성(DB연결)
		loginDao.setConnection(conn);
		// loginDao 인스턴스에서 사용할 Connection 객체를 전달함
		MemberInfo loginMember = loginDao.getLoginMember(uid,pwd);
		// loginDao 인스턴스의 getLoginMember() 메소드를 실행시키고, 그 결과를 MemberInfo형 인스턴스 loginMember에 저장
		close(conn);
		// DB연결끊기
